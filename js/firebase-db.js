...
const API = {
  ...
  async addTransaction(txObj) {
    ...
    return { success: true, message: 'บันทึกรายการสำเร็จ', tx_id: txObj.tx_id };
  },

  async deleteTransaction(txId) {
    const docRef = db.collection('transactions').doc(txId);
    const doc = await docRef.get();
    if (!doc.exists) return { success: false, message: 'ไม่พบรายการนี้' };
  ...
  async addWeek(weekObj) {
    ...
    return { success: true, message: 'เพิ่มสัปดาห์ใหม่สำเร็จ' };
  },

  async deleteWeek(weekId) {
    await db.collection('weeks').doc(weekId).delete();
    return { success: true, message: 'ลบสัปดาห์สำเร็จ' };
  },

  async updateClassSettings(settings) {
    await db.collection('settings').doc('global').set(settings, { merge: true });
    return { success: true, message: 'บันทึกการตั้งค่าห้องเรียนสำเร็จ', settings: settings };
  },
  ...
  async addBatchIncome(payload) {
    ...
    await batch.commit();
    return { success: true, message: 'บันทึกข้อมูลเก็บเงินสำเร็จ' };
  },
  
  async cancelStudentWeekPayment(studentId, weekId, txId) {
    const docRef = db.collection('transactions').doc(txId);
    const doc = await docRef.get();
    if (!doc.exists) return { success: false, message: 'ไม่พบรายการที่ต้องการยกเลิก' };
  ...
  async addUser(payload) {
    ...
    return { success: true, message: 'เพิ่มนักเรียนสำเร็จ' };
  },
